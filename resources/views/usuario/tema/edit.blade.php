                ...
                <div class="row">
                    ...
                    <div class="col-lg">
                        <div class="p-5">
                            ...
                            <form class="user" action="{{ route('usuario.tema.update',$tema->id) }}" method="POST">
                                {{ csrf_field() }}
                                <div class="form-group">
                                    <select class="form-select" name="id_materia" aria-label="Default select example">
                                        
                                        @foreach ($materias as $materia)
                                            @if ($tema->id_materia == $materia->id)
                                                <option value="{{ $materia->id }}" selected>{{ $materia->nombre }}
                                                </option>
                                            @else
                                ...
                                <div class="form-group">
                                    <input type="text" name="nombre" class="form-control form-control"
                                        id="exampleInputEmail" placeholder="Nombre" value="{{$tema->nombre}}">
                                </div>
