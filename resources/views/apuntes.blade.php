            ...
            <div class="row gx-4 gx-lg-5 justify-content-center">
                <div class="col-md-10 col-lg-8 col-xl-7">
                    <div class="post-heading">
                        <h1>Apuntes del tema: {{ $tema->nombre }}</h1>
                        <h2 class="subheading">{{ $tema->descripcion }}</h2>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row gx-4 gx-lg-5 justify-content-center">
                <div class="col-md-10 col-lg-8 col-xl-7">
                    ...
                    @foreach ($apuntes as $apunte)
                        ...
                        <h2 class="section-heading">{{ $apunte->titulo }}</h2>

                        @if (!is_null($apunte->texto))
                            <p>
                                {{ $apunte->texto }}
                            </p>
                        @endif

                        @if (!is_null($apunte->url_recurso))
                            <p>
                                Recurso:
                            </p>
                            <p>
                                <a href="{{ $apunte->url_recurso }}">{{ $apunte->url_archivo }}</a>
                        ...
                        @if (!is_null($apunte->url_imagen))
                            <p>
                                <img class="img-fluid" src="{{ asset('/storage/' . $apunte->url_imagen) }}"
                                    alt="..." />
                            </p>
                        @endif
                        ...
                        @if (!is_null($apunte->url_archivo))
                            ...
                            <p>
                                <a
                                    href="{{ asset('/storage/' . $apunte->url_archivo) }}">{{ $apunte->url_archivo }}</a>
                            </p>
                        @endif
